...
RUN apt-get update && apt-get install -y device-tree-compiler libboost-system-dev \
    libboost-regex-dev autoconf automake autotools-dev curl python3 pypy3 pypy3 pypy3-dev\
    ...
    pypy3 -m pip install --no-cache-dir salabim pycachesim --break-system-packages && \
