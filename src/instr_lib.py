...
class Instr(sim.Component):
    ...
    def store_queue(self):
        ...
        yield self.wait(self.ls_ready)
        self.pe.konata_signature.print_stage(
            "DWU", "RDY", self.pe.thread_id, self.instr_id
        )
        ...
        if self.pe.ResInst.store_queue:
            try:
                self.pe.ResInst.store_queue[
                    self.pe.ResInst.store_queue.index(self) + 1
                ].ls_ready.set(True)
            except IndexError:
                pass
        if not self.store_buff:
        ...
        yield from self.data_cache_pipeline()
        # Make ready any store that could be allocated after issuing this one.
        try:
            self.pe.ResInst.store_queue[
                self.pe.ResInst.store_queue.index(self) + 1
            ].ls_ready.set(True)
