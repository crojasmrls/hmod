    ...
    def process(self):
        ...
        if self.decoded_fields.instr_tuple[dec.INTFields.LABEL] in dec.InstrLabel.ARITH:
            self.konata_signature.print_stage(
                "ALL", "QUE", self.thread_id, self.instr_id
            )
        if self.decoded_fields.instr_tuple[dec.INTFields.LABEL] in dec.InstrLabel.LS:
            self.konata_signature.print_stage(
                "ALL", "LSB", self.thread_id, self.instr_id
            )
        ...
        if (
            self.decoded_fields.instr_tuple[dec.INTFields.LABEL] is dec.InstrLabel.STORE
            and self.resources.RobInst.instr_end(self)
            and not self.back2back
        ):
        ...
        if self.decoded_fields.instr_tuple[dec.INTFields.LABEL] in dec.InstrLabel.ARITH:
            self.konata_signature.print_stage(
                "RRE", "EXE", self.thread_id, self.instr_id
            )
            if (
                self.decoded_fields.instr_tuple[dec.INTFields.LABEL]
                is dec.InstrLabel.BRANCH
                ...
                self.bp_hit = (
                    not self.branch_result and not self.bp_take_branch[0]
                ) or (
                    self.branch_result
            ...
            for x in range(self.decoded_fields.instr_tuple[dec.INTFields.LATENCY]):
                if self.decoded_fields.instr_tuple[dec.INTFields.LATENCY] > 1:
                    if (
                        self.decoded_fields.instr_tuple[dec.INTFields.LATENCY] - x - 2
                        == 0
                    ):
                        if self.decoded_fields.instr_tuple[
                            dec.INTFields.DEST
                        ]:  # Set ready bit to issue
                            self.p_dest.reg_state.set(True)
        ...
        if self.decoded_fields.instr_tuple[dec.INTFields.LABEL] in dec.InstrLabel.LS:
            self.konata_signature.print_stage(
                "RRE", "MEM", self.thread_id, self.instr_id
            )
