        ...
        if type(instr_id) == int and type(thread_id) == int:
            if self.konata_dump_on:
                ...
                try:
                    ...
                    self.konata_ids[thread_id].append(konata_id)
                self.fk.write('I\t' + str(konata_id) + '\t' + str(instr_id) + '\t' + str(thread_id) + '\n')
                # Label
                self.fk.write('L\t' + str(konata_id) + '\t0\t' + str(line) + ': ' + instr + '\n')
        ...
        if type(instr_id) == int and type(thread_id) == int:
            if self.konata_dump_on:
                ...
                try:
                    ...
                    raise
                if is_flush:
                    self.fk.write('R\t' + str(konata_id) + '\t' + str(konata_id) + '\t1\n')
                else:
                    self.fk.write('R\t' + str(konata_id) + '\t' + str(konata_id) + '\t0\n')
        else:
            raise TypeError("Instr id and thread id must be integer values!!")

    def print_torture(self, thread_id, instr_id, instr, dest, data, address):
        if type(instr_id) == int and type(thread_id) == int:
            if self.torture_dump_on:
                try:
                    konata_id = self.konata_ids[thread_id][instr_id]
